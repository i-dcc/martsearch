...
class RawDataSort
  ...
  def self.sort( results )
    ...
    case graph_type
      when :Scatter
        sorted_results = self.sort_scatter_data( lresults )
      when :Bar
        sorted_results = self.sort_bar_data( lresults )
      when :Line
        sorted_results = self.sort_line_data( lresults )
      end
  ...
  def self.sort_scatter_data( results )
    processed_results = {}
  ...
  #  sorted_results = {
  #    :headings => [ "Group" => :membership, "Mouse" => :mouse_name, "Time of Day" => [] ],
  #    :results  => {}
  #  }
  #  
  #  results.each do |result|
