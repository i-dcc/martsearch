...
class MartSearchControllerTest < Test::Unit::TestCase
  ...
  context "A MartSearch::Controller object" do
    should "initialize correctly" do
      assert( @ms.is_a?(MartSearch::Controller) )
      assert( @ms.config != nil )
      assert( @ms.cache != nil )
      assert( @ms.index.is_a?(MartSearch::Index) )
    end
    ...
    should "be a singleton class" do
      ...
      assert_equal( @ms.object_id, @second_obj.object_id, "MartSearch::Controller is not a singleton - we've created a second instance!" )
    end
    
    should "have built many DataSources" do
      assert( @ms.config[:datasources].is_a?(Hash) )
      assert( !@ms.config[:datasources].empty? )
      
      @ms.config[:datasources].each do |ds_name,ds|
    ...
    should "have built up the server config correctly" do
      assert( @ms.config[:server].is_a?(Hash) )
      assert( !@ms.config[:server].empty? )
      @ms.config[:server].each do |key,value|
        assert( key.is_a?(Symbol) )
      end
      
      assert( @ms.config[:server][:dataviews]  != nil )
      assert( @ms.config[:server][:datasets]   != nil )
      
      assert( @ms.config[:server][:dataviews].is_a?(Array) )
      assert( @ms.config[:server][:dataviews_by_name].is_a?(Hash) )
      
      assert( @ms.config[:server][:datasets].is_a?(Hash) )
      @ms.config[:server][:datasets].each do |ds_name,ds|
        assert( ds_name.is_a?(Symbol) )
    ...
    should "allow us to perform controlled searches" do
      VCR.use_cassette('test_controller_search_simulation') do
        ...
        assert( @ms.search_data.is_a?(Hash) )
        assert( @ms.search_data.empty? )
        ...
        bad_result = @ms.send( :search_from_fresh_index, @ms.config[:index][:test][:bad_search], 1 )
        assert_equal( false, bad_result )
        
        good_result = @ms.send( :search_from_fresh_index, @ms.config[:index][:test][:single_return_search], 1 )
        assert_equal( true, good_result )
        
        # Now check the 'cachability' of the data
        search_data = BSON.deserialize( BSON.serialize( @ms.search_data ) )
        search_data = search_data.clean_hash if RUBY_VERSION < '1.9'
        search_data.recursively_symbolize_keys!
        assert_equal( search_data, @ms.search_data )
        ...
        grouped_search_terms = @ms.send( :prepare_dataset_search_terms, @ms.search_data.keys )
        
        # Now drive the dataset searches
        dataset_results = @ms.send( :search_from_fresh_datasets, search_data.keys, grouped_search_terms )
        assert( dataset_results.is_a?(TrueClass) || dataset_results.is_a?(FalseClass) )
        
        @ms.search_data.each do |key,value|
    ...
    should "allow us to perform end-to-end searches" do
      VCR.use_cassette('test_controller_search') do
        @ms.send( :clear_instance_variables )
        @ms.cache.clear
        
        fresh_results  = @ms.search( @ms.config[:index][:test][:single_return_search], 1 )
        cached_results = @ms.search( @ms.config[:index][:test][:single_return_search], 1 )
    ...
    should "give a count of genes/items for each of the configured browsing options" do
      VCR.use_cassette( 'test_controller_browse_counts' ) do
        ...
        fresh_counts  = @ms.browse_counts()
        cached_counts = @ms.browse_counts()
    ...
    should "give top-level progress counts for the WTSI MGP" do
      omit_if(
        @ms.datasets[:'wtsi-phenotyping-heatmap'].nil?,
      ...
      VCR.use_cassette( 'test_controller_wtsi_mgp_counts' ) do
        ...
        fresh_counts  = @ms.wtsi_phenotyping_progress_counts()
        cached_counts = @ms.wtsi_phenotyping_progress_counts()
